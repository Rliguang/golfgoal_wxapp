				...
				<li class="flex flex-between align-center golf-seqLi" v-for="(item,index) in sequenceList" :key="index">
					<view class="flex flex-start align-center">
						<view class="golf-avatar">
							<img :src="item.groupAvatar?item.groupAvatar:'../../static/image/default.png'">
						</view>
						<text>{{item.groupName}}</text>
					</view>
					<view class=" reduce flex  flex-between">
						<image @click="reduceLike(item.golfTournamentGroupId)" src="../../static/image/reduce.png" mode=""></image>
						<!-- <picker @change="bindNumChange($event,item.golfTournamentGalleryId)" mode='selector' range-key="name" :value="index" :range="ruleList"> -->
						<text>{{item.totLikeNumber}}</text>
						<image @click="changeGood(item.golfTournamentGroupId)" class="goods" src="../../static/image/goods.png" mode=""></image>
						<!-- </picker> -->
					</view>
				</li>
			</ul>
...
<script>
	export default {
		...
		onLoad(option) {
			this.pkId = option.id
			this.getLikeGroupList()
			uni.getStorage({
				key: "golfTournamentGalleryId",
				success: (res) => {
					this.golfTournamentGalleryId = res.data
				}
			})
		},
		watch: {},
		methods: {
			reduceLike(id) {
				this.sequenceList.map(res => {
					if (id == res.golfTournamentGroupId) {
						if (res.totLikeNumber == 0) {} else {
							res.totLikeNumber -= 1
						}
					}
				})
			...
			getLikeGroupList() {
				this.$api.score.getLikeGroupList({
					data: {
						pkId: this.pkId
					}
				}).then(res => {
			...
			//修改
			bindSaveChangesBtn() {
				if (this.golfTournamentGalleryId) {
				
					let likeList = []
					this.sequenceList.map(res => {
						likeList.push({
							golfTournamentGroupId: res.golfTournamentGroupId - 0,
							likeNumber: res.totLikeNumber,
							golfTournamentGalleryId: this.golfTournamentGalleryId
						})
					})
					this.$api.score.subLike({
						data: {
							likeList: likeList
						}
					}).then(res => {
